 ...
 */
import { useBlockTheme, HTMLParser, __experimentalUseFieldRenderContext, FieldRenderContextProvider, ErrMsg } from '@quillforms/renderer-core';
...
const GroupDisplay = ({
  ...
  return createElement(Fragment, null, size(innerBlocks) > 0 && innerBlocks.map((block, index) => {
    ...
    refAssigned = true;
    return createElement(FieldRenderContextProvider, {
      key: block.id,
      value: context
    }, createElement("div", {
      className: css`
      ...
      className: classnames('renderer-components-child-block-label', css`
      ...
      value: blockLabel
    })), createElement(Fragment, null, blockType?.display && /* @ts-expect-error */
    createElement(blockType.display, {
      ...blockProps
    })), isErrMsgVisible && answers?.[block?.id]?.validationErr && createElement("div", {
      className: css`
												margin-top: 20px;
											`
